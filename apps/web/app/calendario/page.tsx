...
const getAppointmentStyle = (type: string) => {
    switch (type) {
        case 'Primera Visita': return { color: 'bg-red-50 text-red-600 border-red-500/50', icon: <User size={14} /> };
        case 'Revisión Online': return { color: 'bg-blue-500/20 text-blue-600 border-blue-500/50', icon: <Video size={14} /> };
        case 'Interno': return { color: 'bg-yellow-500/20 text-yellow-500 border-yellow-500/50', icon: <Clock size={14} /> };
        case 'Seguimiento':
        default: return { color: 'bg-slate-50 text-slate-900 border-slate-300', icon: <MapPin size={14} /> };
    }
}
...
export default function CalendarioPage() {
    ...
    return (
        <div className="min-h-[calc(100vh-80px)] bg-white text-slate-900 p-8 overflow-hidden flex flex-col">
            {/* Header */}
            <div className="flex justify-between items-center mb-8 shrink-0">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">{monthYear}</h1>
                    <p className="text-slate-600 mt-1">{viewTitle || 'Vista Mensual'}</p>
                </div>
                ...
                <div className="flex items-center gap-4">
                    <div className="flex bg-white border border-slate-200 rounded-xl overflow-hidden">
                        <button
                            ...
                            className={`px-4 py-2 transition-colors border-r border-slate-200 ${viewMode === 'day' ? 'bg-slate-50 text-slate-900 font-medium' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}>
                            Día
                        </button>
                        <button
                            onClick={() => setViewMode('week')}
                            className={`px-4 py-2 transition-colors ${viewMode === 'week' ? 'bg-slate-50 text-slate-900 font-medium' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}>
                            Semana
                        </button>
                        <button
                            onClick={() => setViewMode('month')}
                            className={`px-4 py-2 transition-colors border-l border-slate-200 ${viewMode === 'month' ? 'bg-slate-50 text-slate-900 font-medium' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-50'}`}>
                            Mes
                        </button>
                    </div>

                    <div className="flex gap-2">
                        <button onClick={prevWeek} className="p-2 border border-slate-200 rounded-xl bg-white text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors">
                            <ChevronLeft size={20} />
                        </button>
                        <button onClick={goToday} className="px-4 py-2 border border-slate-200 rounded-xl bg-white text-slate-900 font-semibold hover:bg-slate-50 transition-colors">
                            Hoy
                        </button>
                        <button onClick={nextWeek} className="p-2 border border-slate-200 rounded-xl bg-white text-slate-600 hover:text-slate-900 hover:bg-slate-50 transition-colors">
                    ...
                    <button onClick={() => {
                        ...
                        setIsModalOpen(true);
                    }} className="bg-red-600 text-black px-4 py-2 rounded-xl font-bold hover:bg-red-600 transition-colors shadow-sm ml-4">
                        Nueva Cita
                    </button>
                </div>
            </div>
            ...
            {viewMode === 'month' ? (
                    ...
                    return (
                        <div className="flex-1 bg-white border border-slate-200 rounded-2xl flex flex-col overflow-hidden">
                            <div className="grid grid-cols-7 border-b border-slate-200">
                                {monthDayNames.map(d => (
                                    <div key={d} className="py-3 text-center text-xs font-bold text-slate-500 uppercase tracking-wider border-r border-slate-200 last:border-0">{d}</div>
                                ))}
                            </div>
                            <div
                                ref={monthScrollRef}
                            ...
                            >
                                {weeks.map((week, wi) => (
                                    <div key={wi} className="grid grid-cols-7 border-b border-slate-200 last:border-0" style={{ minHeight: '100px' }}>
                                        {week.map((date, di) => {
                                            if (!date) return <div key={di} className="border-r border-slate-200 last:border-0 bg-white/30" />;
                                            ...
                                            return (
                                                <div
                                                    key={di}
                                                    className={`border-r border-slate-200 last:border-0 p-2 hover:bg-slate-50 transition-colors flex flex-col gap-1`}
                                                ...
                                                >
                                                    <span className={`text-sm font-bold w-7 h-7 rounded-full flex items-center justify-center cursor-pointer ${isToday ? 'bg-red-600 text-black' : 'text-slate-600 hover:text-slate-900'}`}>
                                                        {date.getDate()}
                                                    </span>
                                                    <div className="flex flex-col gap-1 overflow-hidden">
                                                        ...
                                                        {dayAppts.length > 3 && (
                                                            <span className="text-[10px] text-slate-500 font-medium pl-1">+{dayAppts.length - 3} más</span>
                                                        )}
                                                    </div>
                                                </div>
                                            );
                ...
                // ---- WEEK / DAY VIEW ----
                <div className="flex-1 bg-white border border-slate-200/50 rounded-2xl flex flex-col min-h-0 overflow-hidden">
                    {/* Days Header */}
                    <div
                        className="border-b border-slate-200/60 bg-white shrink-0 grid"
                        style={{ gridTemplateColumns: viewMode === 'day' ? '56px 1fr' : '56px repeat(7, 1fr)' }}
                    >
                        <div className="w-14 border-r border-slate-200/60 flex items-end justify-center pb-3">
                            <span className="text-[9px] font-medium text-slate-500">GMT+1</span>
                        </div>
                        {displayDates.map((date, i) => {
                            const isToday = formatDateToStr(new Date()) === formatDateToStr(date);
                            return (
                                <div key={i} className={`py-3 border-r border-slate-200/60 flex flex-col items-center ${i === displayDates.length - 1 ? 'border-r-0' : ''}`}>
                                    <div className="text-[11px] font-medium text-slate-500 mb-1 uppercase tracking-wider">{(dayNames[date.getDay()] ?? '').slice(0, 3)}</div>
                                    <div className={`text-2xl font-bold w-9 h-9 flex items-center justify-center rounded-full ${isToday ? 'bg-red-600 text-black' : 'text-slate-900'
                                        }`}>{date.getDate()}</div>
                                </div>
                            );
                        })}
                        ...
                        >
                            ...
                            <div className="absolute inset-0 pointer-events-none">
                                {timeSlots.map((time, i) => (
                                    ...
                                    >
                                        ...
                                        {i % 2 === 1 && <div className="absolute inset-0 bg-white" />}
                                        {/* Hour divider line */}
                                        <div className="absolute bottom-0 left-14 right-0 border-b border-slate-200/40" />
                                        {/* Half-hour divider */}
                                        <div className="absolute border-b border-slate-200/20 border-dashed left-14 right-0" style={{ top: '50%' }} />
                                    </div>
                                ))}
                            </div>

                            {/* Time Labels Column */}
                            <div className="border-r border-slate-200/60 relative z-10 w-16">
                                {timeSlots.map((time, i) => {
                                    ...
                                    return (
                                        <div key={time} className="h-[70px] relative">
                                            <span className="absolute -top-[9px] right-3 text-[11px] font-medium text-slate-600 select-none uppercase whitespace-nowrap tabular-nums tracking-tight">
                                                {i === 0 ? '' : displayTime}
                                            </span>
                                        </div>
                                    );
                            ...
                            {displayDates.map((date, arrayIndex) => {
                                ...
                                return (
                                    <div
                                        key={dayIndex}
                                        className={`border-r border-slate-200/40 relative last:border-r-0 ${isToday ? 'bg-red-600/[0.02]' : ''
                                            }`}
                                    ...
                                    >
                                        ...
                                        {isToday && (
                                            <div
                                                className="absolute left-0 right-0 border-t-2 border-red-400 z-10 w-full"
                                                style={{ top: `${currentTimePercent}%` }}
                                            >
                                                <div className="absolute -left-2 -top-[5px] w-2 h-2 rounded-full bg-red-600" />
                                            </div>
                                        )}
                                        ...
                                        {appointments.filter(a => a.dateStr === dateString || (!a.dateStr && a.day === dayIndex && Math.abs(currentWeekDate.getTime() - new Date().getTime()) < 604800000)).map(apt => {
                                            ...
                                            return (() => {
                                                const iconSize = 10;
                                                const currentIcon = apt.type.toLowerCase().includes('online') ? <Video size={iconSize} className="text-blue-600" /> :
                                                    apt.type.toLowerCase().includes('presencial') ? <MapPin size={iconSize} className="text-emerald-600" /> :
                                                        <Clock size={iconSize} className="text-yellow-600" />;
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        ...
                                                        <div className="flex justify-between items-center gap-2 w-full leading-none mb-0.5">
                                                            <div className="font-bold text-[11.5px] text-slate-900 leading-tight" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 'calc(100% - 22px)' }}>{apt.patient}</div>
                                                            <button
                                                                type="button"
                                                                onClick={(e) => {
                                                                    e.preventDefault();
                                                                ...
                                                                className="shrink-0 w-4 h-4 flex items-center justify-center bg-black/10 hover:bg-black/30 rounded text-slate-900/40 hover:text-red-600 transition-colors z-10"
                                                                title="Eliminar Cita"
                                                            >
                                                                <Trash2 size={11} />
                                                            </button>
                                                        ...
                                                        <div className="flex items-center gap-1.5 leading-none mb-0.5" style={{ minHeight: '12px' }}>
                                                            ...
                                                            <span className="text-[10px] font-medium text-slate-900/75" style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{apt.type}</span>
                                                        </div>

                                                        {/* Bottom row: Time */}
                                                        <div className="mt-auto text-[10px] font-semibold text-slate-900/90 leading-none">
                ...
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
                    <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-xl w-full max-w-lg">
                        <div className="flex justify-between items-center mb-6">
                            <h2 className="text-xl font-bold text-slate-900">{editingAppointment ? 'Editar Cita' : 'Agendar Nueva Cita'}</h2>
                        ...
                        <form onSubmit={handleSave} className="space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-slate-600 mb-1">Paciente / Título</label>
                                {formData.type === 'Interno' ? (
                                    ...
                                    <div className="relative">
                                        <select
                                            ...
                                            value={formData.patient}
                                            onChange={(e) => setFormData({ ...formData, patient: e.target.value })}
                                            className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-red-400 appearance-none"
                                        >
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-slate-900 focus:outline-none focus:border-red-400 [color-scheme:dark]"
                                    />
                                </div>
                                <div>
                            ...
                            <div>
                                <label className="block text-sm font-medium text-slate-600 mb-1">Tipo de Cita</label>
                                <select
                                    value={formData.type}
            ...
            {deletingAppointment && (
                <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60]">
                    <div className="bg-white border border-slate-200 p-6 rounded-2xl shadow-xl w-full max-w-sm text-center">
                        ...
                        <h2 className="text-xl font-bold text-slate-900 mb-2">¿Eliminar Cita?</h2>
                        <p className="text-slate-600 mb-6 text-sm">Esta cita se eliminará permanentemente de tu calendario.</p>
                        <div className="flex gap-3">
                            <button onClick={() => setDeletingAppointment(null)} className="flex-1 bg-slate-50 hover:bg-slate-100 text-slate-900 font-medium py-3 rounded-xl transition-colors">
                                Cancelar
                            </button>
                            <button onClick={() => handleDelete(deletingAppointment)} className="flex-1 bg-red-600 hover:bg-red-600 text-slate-900 font-bold py-3 rounded-xl transition-colors shadow-sm">
                                Sí, Eliminar
                            </button>
                        </div>
                    </div>
